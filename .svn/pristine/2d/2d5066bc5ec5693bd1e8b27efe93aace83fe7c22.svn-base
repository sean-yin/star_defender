...
public class ElasticClient {
    ...
    public SearchResponse searchByMuiltiParams(String indexName, String type, ESLogParamsDTO esLogParamsDTO, int from, int size){
        ...
        sourceBuilder.from(from);
        sourceBuilder.size(size);
        sourceBuilder.fetchSource(new String[]{"http_host","@timestamp","upstream_statcode","upstream_resptime","request"}, new String[]{});
        BoolQueryBuilder boolBuilder = QueryBuilders.boolQuery();
        //系统
        MatchPhraseQueryBuilder matchPhraseQueryBuilder = QueryBuilders.matchPhraseQuery("http_host", esLogParamsDTO.getHttp_host()) ;
        boolBuilder.must(matchPhraseQueryBuilder);
        if (StringUtils.isNotBlank(esLogParamsDTO.getRequest())) {
            //请求路径-动作
            MatchPhraseQueryBuilder requestBuild = QueryBuilders.matchPhraseQuery("request", esLogParamsDTO.getRequest());
            boolBuilder.must(requestBuild);
        } else if (CollectionUtils.isNotEmpty(esLogParamsDTO.getRequestList())) {
            BoolQueryBuilder boolShould = QueryBuilders.boolQuery();
            esLogParamsDTO.getRequestList().forEach(requst -> {
                boolShould.should(QueryBuilders.matchPhraseQuery("request", requst));
            });
            boolBuilder.must(boolShould);
        }
        ...
        if (null != esLogParamsDTO.getLteTimestamp() || null != esLogParamsDTO.getGteTimestamp()) {
            ...
            if (null != esLogParamsDTO.getLteTimestamp()) {
                rangeQueryBuilder.lte(StarDateUtils.getNHour(esLogParamsDTO.getLteTimestamp(),8));
            }
            if (null != esLogParamsDTO.getGteTimestamp()) {
                rangeQueryBuilder.gte(StarDateUtils.getNHour(esLogParamsDTO.getGteTimestamp(),8));
            }
            boolBuilder.must(rangeQueryBuilder);
        }
