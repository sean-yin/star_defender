        ...
        if(response.status==1000){
        	new Vue({}).$alert('登录已过期，请重新登录', '提示', {
            	callback:function(action, instance){
                    parent.location.href="/site/login";
            	}
            });
...
(function(){
	...
	//加载按钮权限映射
	/*var menuButtonsMap={};
	Vue.directive('auth-code', {
		bind:function(el, binding,vnode){
			el.style.display=menuButtonsMap[binding.value]!=null?'':'none';
		},
		inserted:function(){},
		update:function(){},
	...
	$.ajax({
	   url:"/hradmin/common/getMenuCodeMap",
	   type:'GET',
	   async:false,
	   data:{},
	   dataType:'json', 
	   ...
	   error:function(xhr,textStatus){
		   console.log('获取按钮权限失败:' + resultMessage);
	   }
	});*/
	/*提供公用定义,方法*/
	Vue.extendData=function(config){
		...
		//登录用户相关变量
		config.data.loginUser=config.data.loginUser||{'id':0,'name':'','contextPath':'/'};
		//分页相关属性
        ...
        config.methods.currentRowChange=config.methods.currentRowChange||function(val){
   		...
   		config.methods.logout=function(){
